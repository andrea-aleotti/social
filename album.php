...
<?php include "assets/parts/header.php"; ?>
    ...
    <main>
        ...
        if (isset($_GET["error"])) {
            switch ($_GET["error"]) {
                case "img-none":
                    ...
                    echo "<span>Foto eliminata</span>";
                    echo "<button type='button' class='btn-close' data-bs-dismiss='alert' aria-label='Close'></button>";
                    echo "</div>";
                    break;
                case "img-wrong":
                    echo "<alert alert-danger alert-dismissible fade show mt-4' role='alert'>";
                    echo "<span>Qualcosa è andato storto: immagine non eliminata.</span>";
                    echo "<button type='button' class='btn-close' data-bs-dismiss='alert' aria-label='Close'></button>";
                    echo "</div>";
                    break;
                case "title-none":
                    echo "<div class='alert alert-success alert-dismissible fade show mt-4' role='alert'>";
                    echo "<span>Titolo modificato</span>";
                    echo "<button type='button' class='btn-close' data-bs-dismiss='alert' aria-label='Close'></button>";
                    echo "</div>";
                    break;
                case "title-wrong":
                    echo "<alert alert-danger alert-dismissible fade show mt-4' role='alert'>";
                    echo "<span>Qualcosa è andato storto: titolo non modificato.</span>";
                    echo "<button type='button' class='btn-close' data-bs-dismiss='alert' aria-label='Close'></button>";
                    echo "</alert>";
                    break;
            }
        ...
        <div class="container-lg my-5 row mx-auto">
            ...
            $conn = openCon();
            $sql = "SELECT * FROM TImages WHERE Eliminata = 0 ORDER BY ImageID DESC LIMIT 10";
            $query = mysqli_query($conn, $sql);
            if ($query) {
                while ($row = mysqli_fetch_assoc($query)) {
            ?>
                    <div class="col-sm-12 col-md-6 col-lg-4 px-0 card mx-2 my-3 gallery-card">
                        <div class="elimina">
                            <?php
